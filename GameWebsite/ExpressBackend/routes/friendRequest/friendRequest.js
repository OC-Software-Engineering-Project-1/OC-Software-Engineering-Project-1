...
router.get("/friendRequests", async function (req, res) {
  try {
    const friendRequests = await FriendRequest.find().populate(
      "requestor recipient"
    );
...
router.post("/friendRequests", async function (req, res) {
  ...
  try {
    if (req.body.recipient === req.body.requestor) {
...
router.post("/friendRequests/:id/accept", async function (req, res) {
  const _id = req.params.id;
  try {
    const request = await FriendRequest.findOne({_id ,status:"PENDING"});
   console.log(request)
    if (!request) {
      return res.status(404).send();
    }
    ...
    if (!user.friendsList.includes(recipientId)) {
    } else {
      user.friendsList.push({ _id: recipientId });
    }
    if (!friend.friendsList.includes(requestorId)) {
    } else {
