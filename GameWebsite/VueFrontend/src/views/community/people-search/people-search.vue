<template>
	<div>
		<div>
			<div class="font-weight-bold">Search for Player</div>
			<input type="text" name="user-search" placeholder="username or email"/>
			<button>Go!</button>
			<!--this search bar would have looked nicer and been functional if there had been more time and if these pages had database access-->
		</div>
		<div class="m-3 p-3 list-entry" v-for="(u, index) in allUsers" :key="index">
			<span class="float-left mr-3 image-placeholder"><!--the profile image would go here--></span>
			<span class="font-weight-bold">{{ u.username }}</span>
			<button class="btn-orange float-right px-2 ml-3">
			...
			<button class="btn-orange float-right px-2 ml-3">
				...
				<span v-else-if="u.alreadyFriends">
			...
			<br />
			<span>mutual friends: {{ u.mutualFriendsCount }}</span>
		</div>
	</div>
</template>

<script src="./people-search.js"></script>
